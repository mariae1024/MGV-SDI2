...
$sql = "SELECT * FROM orders WHERE status_supplier = 'Ready' AND status_delivery NOT LIKE 'Delivered' AND (delivery_id = '0' OR delivery_id = $did) ORDER BY status_delivery LIMIT $page, $number_of_rows_per_page"; 
      ...
      <div class="row">
          ...
          <div class="sidebar-sticky">
            <ul class="nav flex-column">
               <li class="nav-item">
                <a class="nav-link active" href="allordersdeliveryperson.php">
                  <span class="fa fa-fw fa-file"></span>
                  All Orders
                </a>
              </li>
              <li class="nav-item">
                <a class="nav-link" href="ordersDeliveryPerson.php">
		            ...
		            while($r = mysqli_fetch_assoc($res)):?>
		            	<tr> 
                            ...
                            <?php $status = $r['status_delivery'];
                             ...
                             <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#takeorder<?php echo $r['id']; ?>">Take</button><?php } else {  ?>
                                <button type="button" class="btn btn-secondary" data-toggle="modal" data-target="#takeorder<?php echo $r['id']; ?>" disabled>Take</button> <?php } ?>
                            </td>                            
 <div class="modal fade" id="takeorder<?php echo $r['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="takeorder" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalCenterTitle">Confirm take order action</h5>
          ...
          Are you sure you want to take this order?
             ...
             <a href="takeorder.php?id=<?php echo $r['id']; ?>"><button type="button" class="btn btn-primary">Take</button> 
              </a>   
          </div>
        </div>
      </div>
...
<script>
    ...
    var id=$(this).data('id');
    $('#modalTake').attr('href',' takeorder.php?id='+id);
      })
    </script>
